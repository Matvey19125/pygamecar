...
def park():
    ...
    nps_car = ['image/potok_car1.png', 'image/potok_car2.png', 'image/potok_car3.png',
               'image/potok_car4.png', 'image/potok_car5.png', 'image/potok_car6.png']
    ...
    while running:
        ...
        if (car_collider.colliderect(nps_one_collider) or car_collider.colliderect(nps_two_collider) or
                car_collider.colliderect(nps_three_collider) or car_collider.colliderect(nps_four_collider) or
                car_collider.colliderect(nps_five_collider) or car_collider.colliderect(nps_six_collider) or
                car_collider.colliderect(nps_seven_collider) or car_collider.colliderect(nps_eith_collider) or
                car_collider.colliderect(nps_nine_collider)):
            lose()
            running = False
        ...
        if car_collider.colliderect(tochka_win_collider):
            ...
            running1 = True
            clock = pygame.time.Clock()
            manager = pygame_gui.UIManager((800, 960))
            font_button = pygame.font.Font(None, 60)
            text_restart = "Далее"
            text_menu = "В меню"
            button_restart = font_button.render(text_restart, True, (255, 255, 255))
